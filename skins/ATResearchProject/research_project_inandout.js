function rf_inout_selectAllWords(theList) {
...
function rf_inout_addNewKeyword(toList, newText, newValue) {
...
function rf_inout_moveKeywords(fromList,toList,selectThese) {
  theFromList=document.getElementById(fromList);
  for (var x=0; x < theFromList.length; x++) {
    if (theFromList[x].selected) {
      rf_inout_addNewKeyword(toList, theFromList[x].text, theFromList[x].value);
    }
  }
  ...
  rf_inout_selectAllWords(selectThese);
}
